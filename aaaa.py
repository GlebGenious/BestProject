...
try:
    # Ввод размера поля
    размер_поля = int(input("Введите размер поля (советую не больше 11): "))
    ...
    while True:
        ...
        try:
            предположенный_ряд_или_gg = input(
                "Предполагаемая строка (0-{}), или напишите gg, если хотите сдаться: ".format(размер_поля - 1))
            предположенная_колонка_или_gg = input(
                "Предполагаемая колонка (0-{}), или напишите gg, если хотите сдаться: ".format(размер_поля - 1))

            # Проверка команды "gg" для сдачи и завершения игры
            if предположенный_ряд_или_gg == 'gg' or предположенная_колонка_или_gg == 'gg':
            ...
            print("Произошла ошибка. Проверьте правильность ввода числовых значений.")
            continue
